-- =============================================================================
-- Migración ES (segura) — MyWorksApp
-- Corrige: is_admin() / trigger protect no deben ejecutarse durante RENAME/UPDATE
-- Esquema base: profiles, workers, jobs, ... (+ password_reset_codes, tickets)
-- =============================================================================

BEGIN;

-- -----------------------------------------------------------------------------
-- 0) Desactivar trigger que llama a is_admin() ANTES de renombrar
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 1) Renombrar tablas (idempotente si ya existen nombres ES)
-- -----------------------------------------------------------------------------
DO $$
BEGIN
...
-- tickets_soporte (ya snake_case parcial)
...
-- -----------------------------------------------------------------------------
-- 3) Recrear is_admin() CONTRA perfiles (ANTES de reactivar trigger / UPDATEs)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4) Actualizar valores a español (trigger aún desactivado)
-- -----------------------------------------------------------------------------
UPDATE public.perfiles SET rol = CASE rol
  WHEN 'user' THEN 'usuario'
  WHEN 'worker' THEN 'trabajador'
...
UPDATE public.trabajos SET estado = CASE estado
  ...
  WHEN 'no_show' THEN 'no_asistio'
  WHEN 'awaiting_payment' THEN 'esperando_pago'
  WHEN 'awaiting_quotes' THEN 'esperando_cotizaciones'
  WHEN 'quote_selected' THEN 'cotizacion_seleccionada'
  WHEN 'paused_change_order' THEN 'pausado_orden_cambio'
  WHEN 'awaiting_client_approval' THEN 'esperando_aprobacion_cliente'
  ELSE estado
END;

UPDATE public.trabajos SET estado_pago = CASE estado_pago
  WHEN 'none' THEN 'ninguno'
  WHEN 'pending' THEN 'pendiente'
  WHEN 'authorized' THEN 'autorizado'
  WHEN 'held' THEN 'retenido'
...
UPDATE public.trabajos SET modalidad_cobro = CASE modalidad_cobro
  WHEN 'legacy' THEN 'legado'
  WHEN 'fixed_price' THEN 'precio_fijo'
  WHEN 'hourly_block' THEN 'bloque_horas'
  WHEN 'open_quote' THEN 'cotizacion_abierta'
  ELSE modalidad_cobro
END
WHERE modalidad_cobro IS NOT NULL;
...
UPDATE public.pagos SET tipo_pago = CASE tipo_pago
  ...
  WHEN 'change_order' THEN 'orden_cambio'
  WHEN 'overtime' THEN 'horas_extra'
  ELSE tipo_pago
END
...
UPDATE public.disputas SET motivo = CASE motivo
  WHEN 'quality' THEN 'calidad'
  WHEN 'payment' THEN 'pago'
  WHEN 'behavior' THEN 'conducta'
...
UPDATE public.servicios SET categoria = CASE categoria
  ...
  WHEN 'plumbing' THEN 'plomeria'
  WHEN 'electrical' THEN 'electricidad'
  WHEN 'cleaning' THEN 'limpieza'
  WHEN 'assembly' THEN 'ensamblaje'
  WHEN 'tech_support' THEN 'soporte_tecnico'
  WHEN 'gardening' THEN 'jardinera'
  WHEN 'moving' THEN 'mudanza'
  ELSE categoria
END;

UPDATE public.servicios SET modelo_precio = CASE modelo_precio
  WHEN 'hourly' THEN 'por_hora'
  WHEN 'fixed' THEN 'fijo'
  WHEN 'per_item' THEN 'por_item'
  ELSE modelo_precio
END
...
UPDATE public.ordenes_cambio SET estado = CASE estado
  WHEN 'pending_client' THEN 'pendiente_cliente'
...
UPDATE public.acciones_pendientes SET estado = CASE estado
  WHEN 'pending_sync' THEN 'pendiente_sync'
  WHEN 'syncing' THEN 'sincronizando'
  WHEN 'synced' THEN 'sincronizado'
...
UPDATE public.impulsos SET tipo_impulso = CASE tipo_impulso
  WHEN 'visibility' THEN 'visibilidad'
  WHEN 'priority' THEN 'prioridad'
  WHEN 'featured' THEN 'destacado'
  ELSE tipo_impulso
END;

UPDATE public.mensajes SET tipo = CASE tipo
  WHEN 'text' THEN 'texto'
  WHEN 'image' THEN 'imagen'
  ELSE tipo
END;

UPDATE public.fotos_trabajo SET tipo_medio = CASE tipo_medio
  WHEN 'photo' THEN 'foto'
  WHEN 'video' THEN 'video'
  ELSE tipo_medio
END;

UPDATE public.portafolio_trabajador SET tipo_medio = CASE tipo_medio
  WHEN 'photo' THEN 'foto'
  WHEN 'video' THEN 'video'
  ELSE tipo_medio
END;
...
-- -----------------------------------------------------------------------------
-- 5) Trigger Auth + protect (ya con columnas ES)
-- -----------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_trabajos_id_usuario ON public.trabajos (id_usuario);
CREATE INDEX IF NOT EXISTS idx_trabajos_id_trabajador ON public.trabajos (id_trabajador);
CREATE INDEX IF NOT EXISTS idx_trabajos_creado_en ON public.trabajos (creado_en DESC);
CREATE INDEX IF NOT EXISTS idx_disputas_estado ON public.disputas (estado);
CREATE INDEX IF NOT EXISTS idx_pagos_id_trabajo ON public.pagos (id_trabajo);
CREATE INDEX IF NOT EXISTS idx_notificaciones_id_usuario ON public.notificaciones (id_usuario);
CREATE INDEX IF NOT EXISTS idx_mensajes_id_trabajo ON public.mensajes (id_trabajo);
...
CREATE INDEX IF NOT EXISTS idx_trabajadores_categoria ON public.trabajadores (categoria_servicio);
...
COMMENT ON TABLE public.trabajadores IS 'Perfil profesional del trabajador';
COMMENT ON TABLE public.servicios IS 'Catálogo de servicios ofrecidos';
COMMENT ON TABLE public.trabajos IS 'Solicitudes / trabajos entre cliente y trabajador';
COMMENT ON TABLE public.pagos IS 'Pagos e escrow asociados a trabajos';
COMMENT ON TABLE public.disputas IS 'Disputas abiertas por cliente o trabajador';
COMMENT ON TABLE public.mensajes IS 'Chat por trabajo';
COMMENT ON TABLE public.notificaciones IS 'Notificaciones in-app';
