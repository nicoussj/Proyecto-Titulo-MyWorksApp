-- =============================================================================
-- Diagnóstico + continuación segura del rename ES
-- NO ejecutes el dump CREATE TABLE (ese sí falla con "profiles already exists")
-- =============================================================================

-- 1) DIAGNÓSTICO (ejecuta solo este bloque primero y mira el resultado)
SELECT
...
-- Si ves profiles=profiles y perfiles=NULL → BD aún EN; corre el bloque CONTINUAR abajo.
-- Si ves profiles=NULL y perfiles=perfiles → rename de tabla ya hecho; corre CONTINUAR (columnas/valores).
-- Si AMBOS no son NULL → estado raro; corre el bloque AMBOS primero.

-- =============================================================================
-- 2) AMBOS existen (solo si el diagnóstico lo muestra)
-- =============================================================================
/*
DO $$
DECLARE
...
BEGIN
  ...
  IF to_regclass('public.perfiles') IS NOT NULL THEN
    EXECUTE 'SELECT count(*) FROM public.perfiles' INTO n_perfiles;
  END IF;

  RAISE NOTICE 'profiles rows=%, perfiles rows=%', n_profiles, n_perfiles;

  -- perfiles vacío y profiles con datos → borrar perfiles huérfano
  IF n_perfiles = 0 AND n_profiles > 0 AND to_regclass('public.perfiles') IS NOT NULL THEN
    DROP TABLE public.perfiles;
  -- profiles vacío y perfiles con datos → borrar profiles huérfano
  ELSIF n_profiles = 0 AND n_perfiles > 0 AND to_regclass('public.profiles') IS NOT NULL THEN
    DROP TABLE public.profiles;
  ELSIF n_profiles > 0 AND n_perfiles > 0 THEN
    RAISE EXCEPTION 'Ambas tablas tienen datos. No borro nada automático. Avisa al agente.';
  END IF;
END $$;
*/

-- =============================================================================
-- 3) CONTINUAR (idempotente) — pega ESTO completo en el SQL Editor
-- =============================================================================
...
-- Valores ES (trigger aún dropeado)
